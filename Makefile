...
PROJECT_DIRS:=$(sort $(dir $(wildcard $(addsuffix /Makefile,$(addprefix project/,$(TARGETS))))))
$(PROJECT_DIRS): lib
	@printf "  BUILD   $@\n";
	$(Q)$(MAKE) --directory=$@ OPENCM3_DIR=$(OPENCM3_DIR) $(EXAMPLE_RULES)

examples: $(PROJECT_DIRS)
	$(Q)true

examplesclean: $(PROJECT_DIRS:=.clean)
...
.PHONY: build lib examples $(PROJECT_DIRS) install clean stylecheck styleclean \
